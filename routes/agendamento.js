...
router.post("/", isAuthenticated, isBarbeiro, async (req, res) => {
  const { cliente_id, horario, observacao, servico_id } = req.body;

  const barbeiroId = req.session.user.id;
  const servicoId = Number(servico_id);

  if (!cliente_id || !horario || !servicoId) {
    return res.status(400).json({
      erro: "cliente_id, horario e servico_id sao obrigatorios.",
    });
  }
  ...
  try {
    ...
    const [[resolvedServico]] = await conn.query(
