...
const { normalizeTelefone, normalizeDigits } = require("../utils/normalizers");

function emailEhValido(valor) {
  return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(valor);
}

// Valida o payload de edição de barbeiro.
// Todos os campos são opcionais — só valida formato se o campo foi enviado.
// Retorna string de erro ou null.
function validateEdicaoBarbeiroPayload(barbeiro) {
...
module.exports = { validateEdicaoBarbeiroPayload };
